 ...
 * {@link TableInputFormat#AUTO} to sniff one), and an output format, produces the converted text.
 * Never touches whatever the bytes came from — a pasted string, a local file, or a downloaded URL
 * are all read once into memory and the original is never written back to.
 *
 * <p>The actual parsing and writing is delegated to one small class per format
 * ({@link DelimitedTableParser}, {@link JsonTableParser}, {@link MarkdownTableParser},
 * {@link YamlTableParser}, {@link XlsxTableParser}, {@link SqlTableWriter},
 * {@link CsvTableWriter}); this class only resolves {@code AUTO} and dispatches to the right one,
 * which is what makes every input format able to produce every output format without a
...
public final class TableConverter {
     ...
     * checked before anything is decoded as text), then JSON's leading {@code [}/{@code {},
     * then a Markdown table's leading {@code |}, then a YAML-shaped {@code key:} or {@code - }
     * line, finally falling back to CSV or TSV by whichever delimiter is more common on the
     * first line.
     */
    ...
    /** @param includeCreateTable ignored for {@link TableOutputFormat#CSV} */
    public static String write(TableData table, TableOutputFormat format, String tableName, boolean includeCreateTable) {
        return switch (format) {
            case SQL -> SqlTableWriter.write(table, tableName, includeCreateTable);
